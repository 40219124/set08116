...
void divide_triangle(const vector<vec3> &points, int divisions, vector<vec3> &positions, vector<vec4> &colours) {
	...
	if (divisions > 0) {
		...
		vec3 mid = normalize(points[1] + points[2]);
		vec3 left = normalize(points[2] + points[0]);
		// Divide new triangles
		divide_triangle({ points[0], right, left }, divisions - 1, positions, colours);
		divide_triangle({ points[1], mid, right}, divisions - 1, positions, colours);
		divide_triangle({ points[2], left, mid}, divisions - 1, positions, colours);
		divide_triangle({right, mid, left }, divisions - 1, positions, colours);
		// *********************************
	}
	else
	{
...
bool load_content() {
	...
	vector<vec3> v{ normalize(vec3(-sqrtf(3.0f), -1.0f, 0.0f)), normalize(vec3(1.0f, -1.0f, sqrtf(3.0f))), normalize(vec3(1.0f, -1.0f, -sqrtf(3.0f))),
				   vec3(0.0f, 1.0f, 0.0f) };
