...
bool load_content() {
	// *********************************
	// Create a sphere
	sphere = mesh(geometry_builder().create_sphere(50.0f, 50.0f));
	...
	array<string, 6> texs = { "textures/sahara_ft.jpg", "textures/sahara_bk.jpg", "textures/sahara_up.jpg",
		"textures/sahara_dn.jpg", "textures/sahara_rt.jpg", "textures/sahara_lf.jpg" };
	// Create cube_map
	cube_map = cubemap(texs);
...
bool update(float delta_time) {
	...
	cam.set_position(rotate(vec3(0.0f, 1.0f, 10.0f), theta * 0.05f, vec3(0.0f, 1.0f, 0.0f)));
	cam.update(delta_time);
	// *********************************
	// Set skybox position to camera position (camera in centre of skybox)
	skybox.get_transform().position = cam.get_position();
	// rotate the sphere
	sphere.get_transform().rotate(eulerAngleY(half_pi<float>() * delta_time));
	// *********************************
	return true;
}
