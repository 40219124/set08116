...
bool update(float delta_time) {
	...
	if (mov != vec3(0.0f)) {
		mov = normalize(mov) * speed * delta_time;
	}
	target_mesh.get_transform().translate(mov);
	cam.set_target(target_mesh.get_transform().position);
	// Use UP and DOWN to change camera distance
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_UP)) {
		cam.move(-cam.get_distance() * 0.35f * delta_time);
	}
	if (glfwGetKey(renderer::get_window(), GLFW_KEY_DOWN)) {
		cam.move(cam.get_distance() * 0.4f * delta_time);
	}
	// Update the camera
