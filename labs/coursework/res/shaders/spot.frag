...
vec4 calculateSpotLight (in spot_light light, in material mate, in vec3 position, in vec3 normal, in vec3 view_dir, in vec4 tex_col) {
	...
	float att = 1.0f / (light.quadratic * pow(light_distance, 2.0f) + light.linear * light_distance + light.constant);
	float intens = max(dot(-light.direction, light_dir), 0.0f);
	vec4 spot_col = light.light_colour * intens * att;

	float k = max(dot(light_dir, normal), 0.0f);
	vec4 diffuse = k * (mate.diffuse_reflection * spot_col);

	vec3 half_vector = normalize(light_dir + view_dir);
	k = pow(max(dot(half_vector, normal), 0.0f), mate.shininess);
